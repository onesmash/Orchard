...
def _seed_sym(conn, sid, usr, name, mod, target_id="T", kind="function", lang="swift"):
    conn.execute(
        "MERGE (s:Symbol {id: $id}) "
        "SET s.usr=$usr, s.precise_id='', s.name=$name, s.language=$lang, "
        "s.kind=$kind, s.module=$mod, s.target_id=$tid, s.file_path='', "
        "s.signature='', s.container_usr='', s.access_level='public', "
        "s.origin='test', s.is_generated=false",
        {"id": sid, "usr": usr, "name": name, "mod": mod, "lang": lang, "kind": kind, "tid": target_id},
    )
...
def test_architecture_derivation_builds_depends_on(tmp_db_path):
    ...
    _seed_sym(conn, "T:s:UI_view", "s:UI_view", "renderView", "UIKit", target_id)
    _seed_sym(conn, "T:s:Data_repo", "s:Data_repo", "fetchItems", "DataLayer", target_id)
    _seed_sym(conn, "T:s:Same_mod", "s:Same_mod", "helper", "UIKit", target_id)
    ...
    stats = run_architecture_derivation(conn, target_id, build_id="b1")
...
def test_architecture_derivation_includes_references(tmp_db_path):
    ...
    _seed_sym(conn, "T:s:A", "s:A", "funcA", "ModA", target_id)
    _seed_sym(conn, "T:s:B", "s:B", "funcB", "ModB", target_id)
...
def test_architecture_derivation_idempotent(tmp_db_path):
    ...
    stats1 = run_architecture_derivation(conn, target_id, build_id="b1")
    assert stats1["module_deps"] == 1

    stats2 = run_architecture_derivation(conn, target_id, build_id="b1")
...
def test_architecture_derivation_no_cross_module(tmp_db_path):
    ...
    _seed_sym(conn, "T:s:1", "s:1", "f1", "SameMod", target_id)
    _seed_sym(conn, "T:s:2", "s:2", "f2", "SameMod", target_id)
