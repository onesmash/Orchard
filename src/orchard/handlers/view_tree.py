"""get_view_tree — query ViewTree edges from the semantic graph."""
...
@dataclass
class ViewTreeRequest(BaseToolRequest):
...
def get_view_tree(conn, req: ViewTreeRequest) -> BaseToolResponse:
    """Return ViewTree edges (parent view -> child view) from the semantic graph.
    ...
    req
        The view tree query request.

    Returns
    -------
    BaseToolResponse
        ``data`` contains the list of ViewTree edges plus counts.
    """
    ...
    return BaseToolResponse(
        ...
        open_gaps=[] if data else ["no ViewTree edges found"],
    )
